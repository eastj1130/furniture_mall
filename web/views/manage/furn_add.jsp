    ...
    <script type="text/javascript">
        ...
        document.addEventListener("DOMContentLoaded", function () {
            document.getElementById("submitAdd").addEventListener("click", function (event) {
                ...
                let sales = document.querySelector("input[name='sales']").value;
                let stock = document.querySelector("input[name='stock']").value;
                let numReg = /^[0-9]+(\.[0-9]+)?$/;
                if (numReg.test(price)&&numReg.test(sales)&&numReg.test(stock)){
                    return true;
                }
                event.preventDefault();
                document.getElementById("error_msg").innerText = "请输入正确的数据格式";
